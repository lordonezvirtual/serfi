...
- Utiliza fuentes de letra óptimas y de fácil lectura apoyándote de forma estratégica en el formato de negritas de WhatsApp (`*texto*`, un solo asterisco) para resaltar datos clave, saldos, valores o la Tarjeta Olímpica. NUNCA uses doble asterisco `*texto*` ya que en WhatsApp no funciona.
...
def run_banca(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Consultar cuentas y tarjetas del cliente de forma dinámica desde el CRM expuesto por PostgREST
    products_context = ""
    try:
        ...
        if r_cuentas.status_code == 200:
            ...
            if cuentas:
                ...
                for c in cuentas:
                    products_context += f"\n- {c.get('tipo_cuenta', 'Cuenta')}: Saldo ${float(c.get('saldo', 0)):,} COP (Tasa: {c.get('tasa_ea', 0)}% E.A.)"
        ...
        if r_tarjetas.status_code == 200:
            tarjetas = r_tarjetas.json()
            if tarjetas:
                products_context += "\nTarjetas de crédito activas:"
                for t in tarjetas:
                    cupo = float(t.get('cupo_aprobado', 0))
                    deuda = float(t.get('deuda_actual', 0))
                    disponible = cupo - deuda
                    products_context += f"\n- {t.get('tipo_tarjeta', 'Tarjeta')}: Cupo ${cupo:,} COP | Deuda actual ${deuda:,} COP | Disponible ${disponible:,} COP (Estado: {t.get('estado', 'Activa')})"
    except Exception as e:
        print(f"Error fetching dynamic products in run_banca: {e}")
        
    if products_context:
        sys_prompt += f"\n\n[PRODUCTOS Y SALDOS REALES DEL CLIENTE DESDE EL CRM]:{products_context}"
    else:
        # Fallback a los datos estáticos de demo por seguridad
        if client_id == "maria":
...
def run_perfil(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    hitl_task = None
    if "aumento de cupo" in response.content.lower() or "scoring" in response.content.lower() or "pre-aprob" in response.content.lower() or "cupo" in response.content.lower():
        hitl_requested = True
        
        # Recuperar información real y detallada del cliente para la tarea
        profile = get_client_crm_profile.invoke(client_id)
        name = profile.get("name", "Cliente Desconocido")
        segment = profile.get("segment", "Estándar")
        
        original_cupo_str = "$5,000,000 COP"
        proposed_cupo_str = "$6,500,000 COP"
        try:
            r_tarjetas = requests.get(f"{POSTGREST_URL}/tarjetas", params={"cliente_id": f"eq.{client_id}"}, timeout=2.0)
            if r_tarjetas.status_code == 200 and r_tarjetas.json():
                card = r_tarjetas.json()[0]
                cupo = int(card.get("cupo_aprobado", 0))
                original_cupo_str = f"${cupo:,} COP"
                proposed_cupo = int(cupo * 1.3)
                proposed_cupo_str = f"${proposed_cupo:,} COP"
        except Exception:
            pass
        ...
        hitl_task = {
            ...
            "description": f"Pre-aprobación y liberación de cupo en Tarjeta Olímpica basado en scoring crediticio y utilización recurrente para {name}.",
            "original_value": original_cupo_str,
            "proposed_value": proposed_cupo_str,
            "confidence": 98,
            "rag_doc_used": "Tarifario Tarjeta Olimpica 2026",
            "transcript_dialog": f"Agente 360: \"Detectando excelente comportamiento de pago y uso recurrente para {name}. Sugiriendo aumento de cupo del 30%.\""
        }
...
def run_portafolio(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "exención" in response.content.lower() or "tasa preferencial" in response.content.lower() or "13%" in response.content.lower():
        ...
        # Recuperar información real y detallada del cliente para la tarea
        profile = get_client_crm_profile.invoke(client_id)
        name = profile.get("name", "Roberto Gómez Oñate")
        segment = profile.get("segment", "Cliente Preferencial")
        ...
        hitl_task = {
            ...
            "description": f"Excepción de tasa preferencial de SuperCDT al 13.0% E.A. (el límite estándar autorizado es 12.5% E.A.) para retención de fondos de {name}.",
            "original_value": "12.5% E.A.",
            "proposed_value": "13.0% E.A. (Monto $15,000,000)",
            "confidence": 91,
            "rag_doc_used": "Reglamento SuperCDT v3.2",
            "transcript_dialog": f"{name}: \"Si no me mejoran la tasa del CDT al 13%, tendré que retirar los 15 millones de pesos y llevarlos a otro banco que me ofrece mejor rentabilidad.\""
        }
...
def run_retail(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Determinar el día de la semana a partir del último mensaje
    last_user_msg = ""
    for m in reversed(messages):
    ...
    try:
        profile = get_client_crm_profile.invoke(client_id)
        segment = profile.get("segment", "Todos")
    except Exception:
        segment = "Adulto Mayor"
        
    promos = get_active_promotions.invoke({"day_of_week": day_of_week, "segment": segment})
...
def run_ux50(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "dirección" in response.content.lower() or "nueva dirección" in response.content.lower() or "casita" in response.content.lower():
        ...
        # Recuperar información real y detallada del cliente para la tarea
        profile = get_client_crm_profile.invoke(client_id)
        name = profile.get("name", "María Amparo Gutiérrez")
        segment = profile.get("segment", "Adulto Mayor")
        ...
        hitl_task = {
            ...
            "description": f"Cambio de domicilio solicitado vía chat para {name}. Requiere verificación manual por el operador.",
            ...
            "user_speech_audio": False,
            "transcript_dialog": f"{name}: \"...sí mijo, por favor cámbiame la dirección de correspondencia a la Avenida 3 Norte número 23 guion 45, apartamento 402 en la ciudad de Cali...\""
        }
