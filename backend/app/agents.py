...
if OPENAI_API_KEY == "mock-key-for-offline-demo":
    ...
    class MockLLM:
        def invoke(self, messages: List[Any], tools: List[Any] = None) -> AIMessage:
            ...
            if "banca" in system_msg.lower():
                if "saldo" in last_msg:
                    return AIMessage(content="**Agente Banca**: Consultando tu saldo en el Core Bancario...\n- **Cuenta de Ahorros**: $2,450,000 COP disponibles.\n- **Tarjeta de Crédito Olímpica**: Cupo utilizado $3,250,000 COP de un total de $5,000,000 COP (Disponible: $1,750,000 COP).\n¿Deseas ver tus últimos 3 movimientos transaccionales?")
                if "movimiento" in last_msg or "transaccion" in last_msg or "compra" in last_msg:
                    txs = query_client_transactions.invoke("maria") if "maria" in last_msg else query_client_transactions.invoke("carlos")
            ...
            if "perfil 360" in system_msg.lower():
                if "cupo" in last_msg or "aumento" in last_msg:
                    scoring = calculate_credit_scoring.invoke("carlos")
                    return AIMessage(content=f"**Agente Perfil 360**: Evaluando tu scoring crediticio para aumento de cupo...\n{scoring}\n\nHe generado una propuesta formal de **aumento de cupo a $6,500,000 COP** para Carlos Herrera Díaz. Se requiere la firma digital del operador para liberar el cupo en el sistema.")
            ...
            if "portafolio" in system_msg.lower():
            ...
            if "consejero" in system_msg.lower() or "bolsillo" in system_msg.lower():
            ...
            # 6. Caso: Orquestador Central (Supervisor)
            if "saldo" in last_msg:
                return AIMessage(content="[ROUTE] Agente Banca | Solicitud de saldos y cuentas del cliente.")
            if "cupo" in last_msg or "aumento" in last_msg:
